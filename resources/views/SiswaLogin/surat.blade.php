        ...
        <div class="col-md-9">
            <div class="card mt-5">
                ...
                @if(session('berhasil'))
                    ...
                    @endif
                    @if(auth()->check() &&  (auth()->user()->hak_akses == '0') || auth()->guard('siswa')->check())
...
.ui-input-icon svg {
  ...
  height: 20px;
                        ...
                        <tbody>
                            @foreach($surat as $u)
                                <tr>
                                    ...
                                    <td class="text-center">{{$u->tahun_pelajaran}}</td>
                                    <td class="text-center">{{$u->nama_pkl}}</td>
                                    <td class="text-center"><a href="/Siswa/Pengajuan/file/{{$u->id}}" target="_BLANK">Lihat FIle</a></td>
                                    <td>
                                         @if(auth()->check() && auth()->user()->hak_akses == '2')
        @if($u->suratPengantar == '0')
        Belum ada Surat Pengantar
        @else
        <a href="/Siswa/Pengantar/{{$u->id}}" target="_BLANK">Lihat file</a>
        @endif
    @elseif(auth()->check() && (auth()->user()->hak_akses == '0'))
    
        @if($u->suratPengantar == '0')
            Belum ada Surat Pengantar, <a href="/Siswa/Admin/Pengantar/{{$u->id}}" onclick="return confirm('Tambahkan surat pengantar?')">Tambah?</a>
        @else
            <a href="/Siswa/Pengantar/{{$u->id}}" target="_BLANK">Lihat file</a>
        @endif
    ...
    @else
        ...
        @if(!session('berhasil'))
        <script>
            var namaSiswa = '{{ auth()->guard("siswa")->user()->nama_siswa }}';
            var id = '{{$u->id}}';
            Swal.fire({
                icon: 'success',
        ...
        @endif
    @endif
</td>@if(auth()->check() && (auth()->user()->hak_akses == '0'))

                                    <td class="text-center"> <form id="delete-form-{{ $u->id }}" action="{{ route('pengajuan.destroy', $u->id) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
    <button type="submit" class="btn btn-danger" id="delete-btn-{{ $u->id }}" style="display: none;">Hapus</button>
</form>

<a href="#" class="btn btn-danger delete-user" data-id="{{ $u->id }}">
    Hapus
</a> 
                                   <a href="/Siswa/Pengajuan/ubah/{{$u->id}}" class="btn-warning btn">Ubah</a> </td> @endif
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
