                        ...
                        <tbody>
                            @foreach($pkl as $u)
                                <tr>
                                    ...
                                    <td class="text-center">{{$u->quota_pkl}} Siswa</td>
                                    <td class="text-center">@if($u->Status == '1') Valid @else Non Valid @endif</td>
                                    <td class="text-center">{{$u->telp}}</td>
                                    <td class="text-center">{{$u->tahun}}</td>
                                    
                                    <td class="text-center"> @if(auth()->check()) <form id="delete-form-{{ $u->idpkl }}" action="{{ route('pkl.destroy', $u->idpkl) }}" method="POST" style="display: none;">
    @csrf
    @method('DELETE')
    <button type="submit" class="btn btn-danger" id="delete-btn-{{ $u->idpkl }}" style="display: none;">Hapus</button>
</form>

<a href="#" class="btn btn-danger delete-user" data-id="{{ $u->idpkl }}" username="{{ $u->nama_pkl }}">
    Hapus
</a>  <a href="/pkl/edit/{{$u->idpkl}}" class="btn btn-warning">Ubah</a> 
                                    @elseif(auth()->guard('siswa')->check()) @if($u->pelaku == auth()->guard('siswa')->user()->nisn) <a href="/pkl/hapus/{{$u->idpkl}}" data-id="{{ $u->idpkl }}" class="btn-danger btn delete-pkl">Hapus</a>  <a href="/pkl/edit/{{$u->idpkl}}" class="btn btn-warning">Ubah</a> @else - @endif @endif </td>
                                </tr>
                            @endforeach
                        </tbody>
                    </table>
